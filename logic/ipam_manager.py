...
def save_subnets(subnets):
    ...
    try:
        ...
        messagebox.showerror(
            "Erro ao Salvar", f"Não foi possível salvar os dados de IPAM: {e}"
        )
...
def add_subnet(subnet_cidr, description):
    ...
    try:
        ...
        messagebox.showerror(
            "Entrada Inválida",
            f"'{subnet_cidr}' não é uma sub-rede válida. Use o formato CIDR (ex: 192.168.1.0/24).",
        )
        return
    ...
    if any(s["cidr"] == subnet_cidr for s in subnets):
        messagebox.showwarning(
            "Sub-rede Duplicada", "Esta sub-rede já está no inventário."
        )
        return
...
def get_subnet_details(subnet_cidr):
    ...
    try:
        ...
        if network.prefixlen < 31:
            ip_list.insert(
                0, {"ip": str(network.network_address), "status": "Endereço de Rede"}
            )
            ip_list.append(
                {
                    "ip": str(network.broadcast_address),
                    "status": "Endereço de Broadcast",
                }
            )
