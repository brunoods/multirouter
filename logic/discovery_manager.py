...
def start_discovery(app):
    ...
    if not network_range or not username or not password:
        messagebox.showwarning(
            "Dados Incompletos", "Rede, Utilizador e Senha são obrigatórios."
        )
        return
    ...
    thread = threading.Thread(
        target=discover_in_background, args=(app, network_range, username, password)
    )
    thread.start()
...
def discover_in_background(app, network_range, username, password):
    ...
    try:
        app.after(
            0, update_log, "Passo 1: A procurar por anfitriões ativos (Ping Scan)..."
        )
        nm = nmap.PortScanner()
        ...
        if not active_hosts:
            app.after(
                0, update_log, "Nenhum anfitrião ativo encontrado na gama especificada."
            )
        ...
        app.after(
            0,
            update_log,
            f"Passo 2: Encontrados {len(active_hosts)} anfitriões ativos. A tentar identificar...",
        )
        ...
        for host in active_hosts:
            if host in existing_ips:
                app.after(
                    0, update_log, f"-> {host} já existe no inventário. A ignorar."
                )
                continue
            ...
            for type_name, vendor_module in app.VENDOR_MODULES.items():
                if "Roteador" not in type_name and "Switch" not in type_name:
                ...
                try:
                    with ConnectHandler(**conn_details) as net_connect:
                        ...
                        app.after(
                            0,
                            update_log,
                            f"   SUCESSO! Dispositivo {host} identificado como {type_name}.",
                        )

                        # Coleta detalhes
                        hostname = (
                            net_connect.find_prompt().replace(">", "").replace("#", "")
                        )
                        ...
                        app.after(
                            0,
                            update_log,
                            f"   Dispositivo '{hostname}' adicionado ao inventário.",
                        )
            ...
            if not device_identified:
                app.after(
                    0,
                    update_log,
                    f"   FALHA: Não foi possível conectar a {host} com as credenciais fornecidas ou não é um dispositivo suportado.",
                )
