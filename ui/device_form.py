...
class DeviceForm(tk.Toplevel):
    ...
    def __init__(self, parent, controller, device_to_edit=None):
        ...
        self.title(
            "Adicionar Novo Dispositivo" if not device_to_edit else "Editar Dispositivo"
        )
        ...
        ttk.Label(form_frame, text="Nome Apelido:").grid(
            row=0, column=0, sticky="w", pady=5
        )
        self.name_entry = ttk.Entry(form_frame, width=30)
        self.name_entry.grid(row=0, column=1, pady=5)

        ttk.Label(form_frame, text="Endereço IP:").grid(
            row=1, column=0, sticky="w", pady=5
        )
        self.ip_entry = ttk.Entry(form_frame, width=30)
        self.ip_entry.grid(row=1, column=1, pady=5)

        ttk.Label(form_frame, text="Tipo de Dispositivo:").grid(
            row=2, column=0, sticky="w", pady=5
        )
        self.type_combobox = ttk.Combobox(
            form_frame,
            values=list(self.controller.VENDOR_MODULES.keys()),
            state="readonly",
            width=27,
        )
        ...
        save_button = ttk.Button(
            button_frame, text="Salvar", style="primary", command=self.on_save
        )
    ...
    def on_save(self):
        ...
        if not all(
            ...
            messagebox.showerror(
                "Campos Vazios",
                "Todos os campos, exceto a senha, são obrigatórios.",
                parent=self,
            )
            return
