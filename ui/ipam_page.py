...
class IPAMPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Button(
            ...
            text="Adicionar",
            style="primary",
            command=self.open_add_subnet_form,
        ).grid(row=1, column=0, sticky="ew", padx=(0, 2), pady=5)
        ttk.Button(
            left_frame,
            text="Remover",
            style="danger",
            command=self.delete_selected_subnet,
        ).grid(row=1, column=1, sticky="ew", padx=(2, 0), pady=5)
    ...
    def populate_subnet_list(self):
        ...
        for subnet in self.subnets:
            self.subnet_list.insert(
                tk.END, f"{subnet['cidr']} - {subnet['description']}"
            )
    ...
    def delete_selected_subnet(self):
        ...
        if not selection_indices:
            messagebox.showwarning(
                "Nenhuma Seleção", "Selecione uma sub-rede para remover.", parent=self
            )
            return
        ...
        if messagebox.askyesno(
            "Confirmar Remoção",
            f"Tem a certeza que deseja remover a sub-rede {selected_subnet_cidr}?",
            parent=self,
        ):
...
class AddSubnetForm(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        ttk.Button(
            button_frame, text="Adicionar", style="primary", command=self.on_add
        ).pack(side="left", padx=5)
        ttk.Button(
            button_frame, text="Cancelar", style="danger", command=self.destroy
        ).pack(side="left", padx=5)
