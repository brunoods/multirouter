...
class TemplateForm(tk.Toplevel):
    ...
    def __init__(self, parent, variables):
        ...
        for i, var_name in enumerate(variables):
            ttk.Label(form_frame, text=f"{var_name}:").grid(
        ...
        ttk.Button(
            button_frame, text="Aplicar", style="primary", command=self.on_apply
    ...
    def on_apply(self):
        ...
        self.result = {
            var_name: entry.get() for var_name, entry in self.entries.items()
        }

        # Validação simples para garantir que nenhum campo ficou vazio
        if any(not value for value in self.result.values()):
            messagebox.showwarning(
                "Campos Vazios", "Por favor, preencha todas as variáveis.", parent=self
            )
