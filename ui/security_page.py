...
class SecurityPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- Seleção da ACL ---
        ttk.Label(main_frame, text="Adicionar regra para:").grid(
            row=0, column=0, sticky="w", padx=5, pady=5
        )
        self.selected_acl = tk.StringVar()
        self.acl_combobox = ttk.Combobox(
            main_frame, textvariable=self.selected_acl, width=30, state="readonly"
        )
        self.acl_combobox.grid(
        ...
        ttk.Label(rule_frame, text="Ação:").grid(
        ...
        proto_cb = ttk.Combobox(
            ...
            values=["ip", "tcp", "udp", "icmp"],
            width=15,
        )
        proto_cb.grid(row=1, column=1, sticky="w", padx=5, pady=2)
        proto_cb.set("ip")

        ttk.Label(rule_frame, text="Origem (IP/Rede):").grid(
        ...
        ttk.Label(rule_frame, text="Destino (IP/Rede):").grid(
        ...
        ttk.Label(rule_frame, text="Opções (ex: eq 80):").grid(
        ...
        ttk.Button(
            ...
            style="primary",
            command=lambda: add_acl_rule(self.controller),
