...
class VlansPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Label(config_frame, text="ID da VLAN:").grid(row=0, column=0, sticky="w", pady=5, padx=5)
        self.entry_vlan_id = ttk.Entry(config_frame, width=25)
        self.entry_vlan_id.grid(row=0, column=1, pady=5, padx=5)

        ttk.Label(config_frame, text="Nome da VLAN:").grid(row=1, column=0, sticky="w", pady=5, padx=5)
