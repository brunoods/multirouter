...
def get_acl_rule_config_commands(acl_name, rule_details):
    command = (
        f"access-list {acl_name} {rule_details['action']} {rule_details['protocol']} "
        ...
        results.append(
            {
                ...
                "status": "Conforme",
                "details": "Servidor NTP encontrado.",
            }
        )
    else:
        results.append(
            {
                "rule": "NTP Configurado",
                "status": "Não Conforme",
                "details": 'Nenhum comando "ntp server" encontrado.',
            }
        )
...
def parse_acl_rules(acl_name, acl_type, text):
    ...
    in_acl_block = False
    pattern = re.compile(
        r"^\s*(\d+)\s+(permit|deny)\s+(\S+)\s+(any|host \S+|\S+\s\S+)\s+(any|host \S+|\S+\s\S+)(.*)"
    )
...
def parse_ip_brief(text):
    ...
    pattern = re.compile(
        r"^(\S+)\s+([\d\.]+|unassigned)\s+\w+\s+\w+\s+(up|down|administratively down)\s+(up|down)"
    )
    for line in lines:
        match = pattern.match(line)
        if match:
            status = (
                "admin down"
                if "administratively down" in match.group(3)
                else match.group(3)
            )
            results.append(
                {
...
def parse_config(config_text):
    ...
    if match:
        parsed_data["version"] = match.group(1)
...
def parse_vlans(vlan_output):
    vlan_list = []
    pattern = re.compile(
        r"^(\d+)\s+([\w-]+)\s+(active|suspend|act/unsup)", re.MULTILINE
    )
