...
def get_interface_config_commands(interface_name, description, status, vlan_id=None):
    commands = []
    commands.append(
        f'/interface set [find name="{interface_name}"] comment="{description}"'
    )
    if status == "off":
...
def get_vlan_config_commands(vlan_id, vlan_name, interface_master="bridge"):
    return [
        f"/interface vlan add name={vlan_name} vlan-id={vlan_id} interface={interface_master}"
    ]


def get_acl_rule_config_commands(chain_name, rule_details):
    command = (
        f"/ip firewall filter add chain={chain_name} action={rule_details['action']} "
...
def check_compliance(config_text):
    ...
    if re.search(
        r"/system ntp client set enabled=yes servers=", config_text, re.MULTILINE
    ):
        results.append(
            {
                "rule": "NTP Configurado",
                "status": "Conforme",
                "details": "Cliente NTP encontrado e ativo.",
            }
        )
    else:
        results.append(
            {
                "rule": "NTP Configurado",
                "status": "Não Conforme",
                "details": "Cliente NTP não está configurado ou não está ativo.",
            }
        )
...
def parse_acl_rules(chain_name, chain_type, text):
    ...
    for i, block in enumerate(rule_blocks):
        ...
        if chain_match and chain_match.group(1) == chain_name:
            ...
            protocol = (
                m.group(1) if (m := re.search(r"protocol=(\S+)", block)) else "any"
            )
            src_addr = (
                m.group(1) if (m := re.search(r"src-address=(\S+)", block)) else "any"
            )
            dst_addr = (
                m.group(1) if (m := re.search(r"dst-address=(\S+)", block)) else "any"
            )
            comment = (
                m.group(1) if (m := re.search(r"comment=\'(.*?)\'", block)) else ""
            )
            results.append(
                {
                    "id_term": str(i),
...
def parse_vlans(vlan_output):
    ...
    for match in matches:
        vlan_list.append(
            {"name": match.group(1), "id": match.group(2), "status": "active"}
        )
    return vlan_list
