        ...
        results.append(
            {
                ...
                "status": "Conforme",
                "details": "Configuração de NTP encontrada.",
            }
        )
    else:
        results.append(
            {
                "rule": "NTP Configurado",
                "status": "Não Conforme",
                "details": "Nenhuma configuração de NTP encontrada em [system ntp].",
            }
        )
...
def get_interface_config_commands(interface_name, description, status, vlan_name=None):
    commands = []
    if status == "off":
        commands.append(f"set interfaces {interface_name} disable")
    else:
        commands.append(f"delete interfaces {interface_name} disable")
    if description:
        commands.append(f'set interfaces {interface_name} description "{description}"')
    else:
        commands.append(f"delete interfaces {interface_name} description")
    if vlan_name:
        commands.append(
            f"set interfaces {interface_name} unit 0 family ethernet-switching vlan members {vlan_name}"
        )
    return commands
...
def get_acl_rule_config_commands(filter_name, rule_details):
    ...
    if rule_details.get("destination") and rule_details["destination"] != "any":
        commands.append(
            f"{base_command} from destination-address {rule_details['destination']}"
        )
...
def parse_acl_rules(filter_name, filter_type, text):
    ...
    try:
        filter_block_match = re.search(
            r"filter " + re.escape(filter_name) + r" {(.*?)}", text, re.DOTALL
        )
...
def parse_config(config_text):
    ...
    if match:
        parsed_data["hostname"] = match.group(1)
    for block in re.finditer(r"(\S+-(\d+/\d+/\d+)) {", config_text):
        if_name = block.group(1)
        if_details_match = re.search(
            r"interface " + re.escape(if_name) + r" {(.*?)}", config_text, re.DOTALL
        )
